...
class App {
	...
	_createGui() {
		...
		const folderProperties = gui.addFolder('FogExp2 Properties');
		folderProperties.addColor(this._properties, 'fogColor').onChange((value) => {
			this._scene.fog.color.set(value);
		});
		folderProperties.add(this._properties, 'fogDensity', 0.001, 0.10).step(0.001).onChange((value) => {
