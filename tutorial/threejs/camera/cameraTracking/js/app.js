...
class App {
	constructor(canvas)	{
		...
		this._cameraTracking = new THREE.PerspectiveCamera(85, this._getCameraAspect(), 0.1, 500);
	...
	_createGui() {
		...
		folderCamera.add(this._properties, 'cameraTracking').onChange((value) => {
		});
	...
	_setCameraPositionByDeltaTime(deltaTime) {
		...
		this._cameraTracking.position.copy(pos);
		this._cameraTracking.matrix.lookAt(this._cameraTracking.position, lookAt, this._binormal);
		this._cameraTracking.rotation.setFromRotationMatrix(this._cameraTracking.matrix, this._cameraTracking.rotation.order);
	}
	...
	_render() {
		...
		this._renderer.render(this._scene, this._properties.cameraTracking ? this._cameraTracking : this._camera);
	}
	...
	_onResizeHandler(event) {
		...
		this._cameraTracking.aspect = this._getCameraAspect();
		this._cameraTracking.updateProjectionMatrix();
