...
class App {
	...
	_createGui() {
		...
		const folderProperties = gui.addFolder('Hemisphere Properties');
		folderProperties.addColor(this._properties, 'hemisphereSkyColor').onChange((value) => {
			this._hemisphereLight.color.set(value);
		});
		folderProperties.addColor(this._properties, 'hemisphereGroundColor').onChange((value) => {
			this._hemisphereLight.groundColor.set(value);
		});
		folderProperties.add(this._properties, 'hemisphereIntensity', 0, 1).step(0.01).onChange((value) => {
