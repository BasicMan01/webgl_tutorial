...
class App {
	constructor(canvas)	{
		...
		this._properties = {
			...
			'webCamAudio': false,
			'webCamVideo': true,
			'webCamVideoPlay': () => { document.getElementById('video').play(); },
			'webCamVideoPause': () => { document.getElementById('video').pause(); }
		};
	...
	_createGui() {
		...
		const folderVideo = gui.addFolder('Video this._properties');
		folderVideo.add(this._properties, 'webCamAudio').onChange((value) => {
			this._initVideo();
		});
		folderVideo.add(this._properties, 'webCamVideo').onChange((value) => {
			this._initVideo();
		});
		folderVideo.add(this._properties, 'webCamVideoPlay');
		folderVideo.add(this._properties, 'webCamVideoPause');

		gui.close();
	}

	_initVideo() {
		if (this._properties.webCamAudio === false && this._properties.webCamVideo === false) {
		...
		navigator.mediaDevices.getUserMedia({
			audio: this._properties.webCamAudio,
			video: this._properties.webCamVideo
		}).then((stream) => {
			this._stream = stream;
